...
from app.services.layout_parsing_client import LayoutParsingClient
...
def get_layout_parsing_client(settings: Settings = Depends(get_settings)) -> LayoutParsingClient | None:
    base_url = settings.layout_parsing_base_url.strip()
    if not base_url:
        return None

    return LayoutParsingClient(
        base_url=base_url,
        timeout_seconds=settings.request_timeout_seconds,
    )
...
def get_pdf_ocr_service(
    settings: Settings = Depends(get_settings),
    openai_client: OpenAIStructuredClient = Depends(get_openai_client),
    layout_parsing_client: LayoutParsingClient | None = Depends(get_layout_parsing_client),
) -> PDFOCRService:
    return PDFOCRService(
        openai_client=openai_client,
        layout_parsing_client=layout_parsing_client,
